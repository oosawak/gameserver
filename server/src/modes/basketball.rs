...
impl GameMode for BasketballMode {
    fn init(&mut self, world: &mut World) {
        ...
        println!("🏀 Basketball Mode initialized!");
    }
        ...
        if let Some(entity) = world.get_entity_mut(input.player_id) {
            ...
            if input.action1 {
                if let Some(ball_id) = self.ball_id {
                    try_shoot_ball(world, input.player_id, ball_id);
                }
            }
        }
    }
    ...
    fn update(&mut self, world: &mut World, dt: f32) {
        ...
        if let Some(ball_id) = self.ball_id {
            check_shot_success(world, ball_id);
        }
    }
...
fn try_shoot_ball(world: &mut World, player_id: EntityId, ball_id: EntityId) {
    ...
    if let Some(ball) = world.get_entity(ball_id) {
        ...
        if dist < player_radius + ball.radius + 30.0 {
            if let Some(ball) = world.get_entity_mut(ball_id) {
                ...
                if len > 0.0 {
                    ball.physics.vx = (dx / len) * 500.0;
                    ball.physics.vy = (dy / len) * 500.0;
                }
            }
        }
    }
...
fn check_shot_success(world: &mut World, ball_id: EntityId) {
    if let Some(ball) = world.get_entity(ball_id) {
        let hoop_y_left = 100.0;
        let hoop_y_right = 900.0;

        if (ball.transform.x - 50.0).abs() < 20.0 && (ball.transform.y - hoop_y_left).abs() < 20.0 {
            println!("🎉 BASKET! Team 1 scored!");
            if let Some(ball) = world.get_entity_mut(ball_id) {
        ...
        } else if (ball.transform.x - 950.0).abs() < 20.0 && (ball.transform.y - hoop_y_right).abs() < 20.0 {
            println!("🎉 BASKET! Team 2 scored!");
            if let Some(ball) = world.get_entity_mut(ball_id) {
                ball.transform.x = 500.0;
                ball.transform.y = 500.0;
                ball.physics.vx = 0.0;
