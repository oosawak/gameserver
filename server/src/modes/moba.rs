use gameserver_core::{
    GameMode, World, PlayerInput, Entity, EntityKind, Transform, Health, gen_entity_id,
};
...
impl GameMode for MobaMode {
    fn init(&mut self, world: &mut World) {
        // スポーン配置
        let player1_transform = Transform::new(100.0, 100.0);
        let player1 = Entity::new(gen_entity_id(), EntityKind::Player, player1_transform, 20.0)
            .with_health(Health::new(100.0));
        world.add_entity(player1);

        let player2_transform = Transform::new(900.0, 900.0);
        let player2 = Entity::new(gen_entity_id(), EntityKind::Player, player2_transform, 20.0)
            .with_health(Health::new(100.0));
        world.add_entity(player2);

        // 壁生成
        world.map.add_wall(400.0, 400.0, 50.0, 200.0);
        world.map.add_wall(550.0, 400.0, 50.0, 200.0);

        println!("🎮 MOBA Mode initialized!");
    }
